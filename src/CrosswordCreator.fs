...
let between min max value = not (value < min || value > max)
...
let isWordPosEmpty (coords: BoardCoord) (word: Word) (dir: LayoutDir) (board: Board) : bool =
    ...
    let rowOrColumnSeq =
        ...
        | Horizontal ->
            (seq {
                for c in cS .. (cS + wordLen) do
                    yield rS, c
            })
        | Vertical ->
            (seq {
                for r in rS .. (rS + wordLen) do
                    yield r, cS
            })
    ...
    let hasTrailingSpace =
        ...
        | Vertical ->
            board.[rS + wordLen, cS] = EmptyChar
            || toUpper board.[rS + wordLen, cS] = toUpper word.[wordLen]
        | Horizontal when btwn (cS + wordLen + 1) -> board.[rS, cS + wordLen + 1] = EmptyChar
        | Horizontal ->
            board.[rS, cS + wordLen] = EmptyChar
            || toUpper board.[rS, cS + wordLen] = toUpper word.[wordLen]
...
let public layoutWord (coords: BoardCoord) (dir: LayoutDir) (word: Word) (board: Board) : Board option =
    ...
    | word -> Some(layoutWordInternal coords dir word board)
...
let addWordToPuzzle (puzzle: Puzzle) (word: InputWord) : seq<Puzzle> =
    seq {
        ...
        | (board, wordCount, wordList) when wordCount = 0 ->
            ...
            let newBoard =
                board
                |> layoutWordInternal coords Horizontal word.Word
        ...
        | (board, wordCount, words) when wordCount > 0 ->
            ...
            for (coords, dir) in res do
                ...
                let idx =
                    words
                    |> Seq.tryFind (fun w -> w.Coord = coords && w.Dir <> dir)
...
let fixSpacesInWords (words: InputWords) =
    words
    |> Seq.map (fun r ->
        { Word = r.Word.Replace(" ", Convert.ToString(SpaceChar))
          Hint = r.Hint })
...
let shrinkPuzzleToSmallest (puzzle: Puzzle) : Puzzle =
    ...
    | (board, wordCount, puzzleWords) ->
        ...
        let emptyLeadingRows =
            seq {
                for row in b..e do
                    yield forAllColumnsInRowAreEmpty row board b e
            }
            |> getTrueCount
        ...
        let emptyLeadingColumns =
            seq {
                for col in b..e do
                    yield forAllRowsInColumnAreEmpty col board b e
            }
            |> getTrueCount
                ...
                for r in b..e do
                    ...
                    yield
                        String.Join(
                            ...
                            seq {
                                for c in b..e do
                                    let tdStyle = """ style="border: 1px solid black; margin: .25em;" """
                                    ...
                                    yield
                                        if cellVal <> EmptyChar then
                                            ...
                                            if wordsByCoords.ContainsKey key then
                                                ...
                                                sprintf
                                                    """<td%s><input type="text" maxlength="1" size="1" placeholder="%d"></input></td>"""
                                                    tdStyle
                                                    w.Index
                                            else
                                                sprintf
                                                    """<td%s><input type="text" maxlength="1" size="1"></input></td>"""
                                                    tdStyle
                                        else
...
let createPuzzles (words: InputWords) (puzzle: Puzzle) =
    let invalidWords =
        words
        |> Seq.where (fun w ->
            System.String.IsNullOrEmpty(w.Word)
            || System.String.IsNullOrEmpty(w.Hint))
        |> Seq.toList
